...
/*
  This sketch has been modified using a different library for oled display management and some improvements have been made, it is still in development, 
  any suggestions are well accepted.
  I thank Ricardo for the excellent work he has done and for making this fantastic library available to everyone.

  CT2JWF
  https://www.youtube.com/channel/UCAD1LHg4uTxMhF3B3sqMl8Q
  ...
  2) SSD1306Ascii (on your Arduino IDE, look for this library on Tools->Manage Libraries).
...
#define AVC_BUTTON 21     // **** Use thi button to implement a new function
#define BAND_BUTTON 17      // Next band
#define SOFTMUTE_BUTTON 15     // **** Use thi button to implement a new function
...
Band band[] = {
 {LW_BAND_TYPE, 100, 510, 300, 0, 4},
  ...
  {SW_BAND_TYPE, 1700, 3500, 1840, 0, 4},     // 160 meters
  {SW_BAND_TYPE, 3500, 4500, 3573, 0, 5},     // 80 meters
  {SW_BAND_TYPE, 4500, 5600, 5357, 0, 4},     // 60 meters
  {SW_BAND_TYPE, 5600, 6800, 6000, 1, 4},
  {SW_BAND_TYPE, 6800, 7400, 7074, 0, 4},     // 40 meters
  {SW_BAND_TYPE, 7400, 8500, 7400, 1, 4},     
  {SW_BAND_TYPE, 8500, 10000, 9600, 1, 4},
  {SW_BAND_TYPE, 10000, 11200, 10136, 0, 4},  // 30 meters
  {SW_BAND_TYPE, 11200, 13400, 11940, 1, 4},
  {SW_BAND_TYPE, 13400, 13900, 13600, 1, 4},
  {SW_BAND_TYPE, 13900, 14500, 14074, 0, 4},  // 20 meters
  {SW_BAND_TYPE, 14500, 17200, 15300, 1, 4},
  {SW_BAND_TYPE, 17200, 18000, 17600, 1, 4},
  {SW_BAND_TYPE, 18000, 21000, 18100, 0, 4},  // 17 meters
  {SW_BAND_TYPE, 21000, 21450, 21074, 0, 4},  // 15 mters
  {SW_BAND_TYPE, 21451, 21880, 21500, 1, 4},  // 13 mters
  {SW_BAND_TYPE, 24880, 26200, 24915, 0, 4},  // 12 meters
  {SW_BAND_TYPE, 26200, 28000, 27500, 1, 4},  // CB band (11 meters)
  {SW_BAND_TYPE, 28000, 30000, 28074, 0, 4},  // 10 meters
  {FM_BAND_TYPE, 6400, 8740, 7000, 3, 0},     // FM from 64 to 87,4MHz; default 70MHz; default step frequency index is 3; default bandwidth index AUTO
  {FM_BAND_TYPE, 8750, 10800, 10220, 3, 0}
};
...
// Configuration; remove/add a double-slash
//////////////////////////// SELECT DISPLAY //////////////////////////////
                                                                        //
// display.begin(&SH1106_128x64, I2C_ADDRESS);  //  1.3 OLED SH1106     //
   display.begin(&Adafruit128x64, I2C_ADDRESS); // 0.96 OLED SSD1306    //
                                                                        //
//////////////////////////////////////////////////////////////////////////
...
//  display.on();
  ...
  if (digitalRead(ENCODER_BUTTON) == LOW)
  {
    ...
    display.print(F("EEPROM RESETED"));
    EEPROM.commit();
    delay(2000);
    display.clear();
  }
  ...
  if (BUTTONEVENT_ISLONGPRESS(event))
  {
    ...
    else
    {
      ...
      if (BUTTONEVENT_FIRSTLONGPRESS == event)
      {
        ...
        {
          bwFlag = (uint8_t*)&bwIdxAM;
          bwMax = 6;
        }
        else if ( FM == currentMode )
        {
          bwFlag = (uint8_t*)&bwIdxFM;
          bwMax = 4;
        }
        else
        {
          bwFlag = (uint8_t*)&bwIdxSSB;
          bwMax = 5;
        }
        if (0 == *bwFlag)
          direction = 1;
...
void showFrequency()
{
  ...
  display.setCursor(93, 0); // KHZ/MHZ
  display.print(unit);
