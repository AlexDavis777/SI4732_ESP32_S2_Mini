...
#include <U8g2lib.h>
// Initialize the U8g2 library with SSD1306 128x64 I2C, using hardware I2C pins
U8G2_SSD1306_128X64_NONAME_F_HW_I2C u8g2(U8G2_R0, U8X8_PIN_NONE);  // No reset pin
UI ui;
...
#define ELAPSED_COMMAND      3000  // time to turn off the last command controlled by encoder. Time to goes back to the VFO control // G8PTN: Increased time and corrected comment
...
void drawMainVFO() {
	//ui.clearMain();
	ui.setBlackColor();
	ui.lcd()->drawBox(0, 9, W, 55);

	ui.drawFrequencyBig(currentFrequency, currentBFO, band[bandIdx].bandType, currentMode, 76, 29);

	//ui.draw_ic_mode(102, 34, WHITE);

	ui.setFont(Font::FONT_6X10);
	ui.drawString(TextAlign::CENTER, 106, 0, 28, false, false, false, getStr(modeStr, currentMode));

	ui.setFont(Font::FONT_5X8);

	//ui.draw_ic24_step(0, 25, WHITE);
	if (band[bandIdx].bandType == FM_BAND_TYPE) {
		ui.drawString(TextAlign::LEFT, 7, 0, 15, false, false, false, getStr(FmStepStr, currentStepIdx));
	}
	else {
		ui.drawString(TextAlign::LEFT, 7, 0, 15, false, false, false, getStr(AmSsbStepStr, currentStepIdx));
	}

	//ui.draw_ic24_bandwidth(42, 25, WHITE);
	if (isSSB())
	{
		ui.drawString(TextAlign::LEFT, 48, 0, 15, false, false, false, getStr(bandwidthSSBStr, bwIdxSSB));
	}
	else if (currentMode == AM)
	{
		ui.drawString(TextAlign::LEFT, 48, 0, 15, false, false, false, getStr(bandwidthAMStr, bwIdxAM));	
	}
	else
	{
		ui.drawString(TextAlign::LEFT, 48, 0, 15, false, false, false, getStr(bandwidthFMStr, bwIdxFM));
	}

	//ui.draw_ic24_agc(90, 25, WHITE);
	if (agcNdx == 0 && agcIdx == 0) {
	 ui.drawString(TextAlign::LEFT, 94, 0, 15, false, false, false, "AGC ON");
	}
	else
	{
		ui.drawStringf(TextAlign::LEFT, 94, 0, 15, false, false, false, "ATT: %2.2d", agcNdx);
	}

	// RSSI
	//ui.drawRSSI(rssi, getStrength(), snr, 1, 55);

	//ui.setFont(Font::FONT_18_TF);
	if (isCW && band[bandIdx].bandType != FM_BAND_TYPE) {
		ui.setFont(Font::FONT_6X10);
		ui.drawString(TextAlign::LEFT, 1, 0, 40, false, false, false, "CW");
	...
	//ui.setFont(Font::FONT_4X6MR);
	//ui.drawStringf(TextAlign::LEFT, 10, 0, 104, true, true, false, "%d - %d", static_cast<int>(mFreq), static_cast<int>(mMag));

	ui.setBlackColor();
	ui.lcd()->drawBox(0, 110, W, 30);

	if (band[bandIdx].bandType == FM_BAND_TYPE) {
		if (rx.getCurrentPilot()) {
			ui.draw_stereo(5, 18, WHITE);
		}

		if (stationName != nullptr) {
			ui.setWhiteColor();
			ui.setFont(Font::FONT_4X6MR);
			ui.drawStringf(TextAlign::CENTER, 0, 30, 8, false, false, false, "%s", stationName);
			if (rdsMsg != nullptr) {
				ui.setFont(Font::FONT_4X6MR);
				ui.draw_string_multi_line(rdsMsg, 33, 90, 34);
			}
		}
		ui.drawRSSI(rssi, getStrength(), snr, 1, 55);
	}
		...
		if (!decodeCW) {
			ui.setFont(Font::FONT_4X6MR);
			ui.drawStringf(TextAlign::CENTER, 80, 125, 60, false, false, false, band[bandIdx].bandName);

			ui.setFont(Font::FONT_4X6MR);
			//ui.draw_start(10, 50, WHITE);
			ui.drawFrequency(band[bandIdx].minimumFreq, 5, 60);

			//ui.draw_finish(10, 50, WHITE);
			ui.drawFrequency(band[bandIdx].maximumFreq, 44, 60);
			ui.drawRSSI(rssi, getStrength(), snr, 1, 55);
		}
		else {
			...
			ui.setWhiteColor();
			ui.setFont(Font::FONT_6X10);
			ui.lcd()->drawStr(1, 50, "Morse");
			ui.lcd()->drawStr(44, 50, morseBuffer);
			//ui.lcd()->drawStr(1, 60, "Decoded");
			ui.lcd()->drawStr(1, 60, textBuffer);
...
void drawSpectrum(int x, int y) {
	...
	ui.setBlackColor();
	ui.lcd()->drawBox(x - 1, y, 128, TOP);

	//uint16_t vuRead = static_cast<int>((peak / 15000) * 130);
	uint16_t vuRead = map(peak, 0, 30, 0, 128) ;
	if (vuRead > 128) vuRead = 128;
	...
	ui.setBlackColor();
	ui.lcd()->drawBox(x - 1, y + TOP + 1, 128, NUM_WATERFALL_ROWS);
	ui.setWhiteColor();
	...
	for (int row = 0; row < NUM_WATERFALL_ROWS; ++row) {
		for (int col = 0; col < NUM_BANDS; ++col) {
			...
			if (color == 1) {
				ui.lcd()->drawPixel(x + col, y + (TOP + 1) + row); //waterfall
			}
		}
	}
...
void showMenuScreen(const char* name, uint8_t h) {
	...
	ui.setBlackColor();
	ui.lcd()->drawRBox(posX - 3, posY, mW + 6, h + 6, 8);
	...
	ui.setFont(Font::FONT_6X10);
	ui.drawString(TextAlign::CENTER, posX + 1, posX + mW - 2, posY + 10, false, false, false, name);
 ...
 */
bool isMenuMode() {
	return (cmdMenu | cmdStep | cmdBandwidth | cmdAgc | cmdVolume | cmdSoftMuteMaxAtt | cmdMode | cmdBand | cmdCal | cmdCwl |cmdAvc);     // G8PTN: Added cmdBand, cmdCal, cmdBrt and cmdAvc
}
...
void setup() {
	...
	pinMode(STEP_BUT, INPUT_PULLUP);
	pinMode(BANDW_BUT, INPUT_PULLUP);
	pinMode(AGC_BUT, INPUT_PULLUP);
	...
	if (digitalRead(MODE_BUT) == LOW)
	{

		ui.showStatusScreen("", "Spectrum Enabled ");
		spectrum = true;
		u8g2.updateDisplay();
		delay(3000);
	}
...
void doCurrentMenuCmd() {
	...
	case SOFTMUTE:                  // SOFTMUTE
		if (currentMode != FM) {
			...
			showInfoMsg("Not available in FM !");
		}
		break;
	case SEEKUP:                    // SEEKUP
	...
	case CALIBRATION:               // CALIBRATION
		if (isSSB()) {
			...
			showInfoMsg("Only available in SSB !");
		}
		break;
		// G8PTN: Added
	case AVC:                       // AVC
...
void drawMenu() {
	int menuSize = 64; //height main menu
	if (isMenuMode()) {
		if (cmdMenu) {
			showMenuScreen("", menuSize);
			ui.drawMenu();
		}
		else if (cmdMode) {
			showMenuScreen("", 50);
			...
			showMenuScreen("AGC/ATTN", 45);
			if (agcNdx == 0 && agcIdx == 0) {
				ui.setFont(Font::SEGMENT7_14);
				ui.drawStringf(TextAlign::CENTER, 66, 95, 34, false, false, false, "ON");
			}
			else {
				ui.setFont(Font::SEGMENT7_14);
				ui.drawStringf(TextAlign::CENTER, 66, 95, 34, false, false, false, "%2.2d", agcNdx);
			}
		}
		else if (cmdBandwidth) {
			if (currentMode == FM) {
			...
			showMenuScreen("Volume", 45);
			ui.draw_ic24_sound_on(45, 19, WHITE);
			ui.setFont(Font::SEGMENT7_14);
			ui.drawStringf(TextAlign::CENTER, 51, 110, 32, false, false, false, "%u", map(rx.getVolume(), 0, 63, 0, 80));
		}
		else if (cmdSoftMuteMaxAtt) {
			showMenuScreen("SoftMute", 40);
			ui.setFont(Font::SEGMENT7_14);
			ui.drawStringf(TextAlign::CENTER, 51, 110, 35, false, false, false, "%2.2d", softMuteMaxAttIdx);
		}
		else if (cmdBand) {
			showMenuScreen("", menuSize);
			ui.drawMenu();
			...
			showMenuScreen("Calibration", 40);
			ui.setFont(Font::SEGMENT7_14);
			ui.drawStringf(TextAlign::CENTER, 51, 110, 32, false, false, false, "%4.4d", currentCAL);
		}
		else if (cmdCwl) {
			showMenuScreen("CW Level", 40);
			ui.setFont(Font::SEGMENT7_14);
			ui.drawStringf(TextAlign::CENTER, 51, 110, 32, false, false, false, "%4.4d", SIGNAL_THRESHOLD);
		}
		else if (cmdAvc) {
			showMenuScreen("AVC", 40);
			ui.setFont(Font::SEGMENT7_14);
			ui.drawStringf(TextAlign::CENTER, 51, 110, 32, false, false, false, "%2.2d", currentAVC);
...
void loop() {
	...
	if (digitalRead(STEP_BUT) == LOW) {
	cmdStep = true;
		if (currentMode == FM) {
			ui.setMenu(currentStepIdx, FmStepStr, 4);
		}
		else if (isSSB()) {
			ui.setMenu(currentStepIdx, AmSsbStepStr, 5);
		}
		else {
			ui.setMenu(currentStepIdx, AmStepStr, 5);
		}
	}
	
	if (digitalRead(BANDW_BUT) == LOW) {
	cmdBandwidth = true;
		if (currentMode == FM) {
			ui.setMenu(bwIdxFM, bandwidthFMStr, 4);
		}
		else if (isSSB()) {
			ui.setMenu(bwIdxSSB, bandwidthSSBStr, 6);
		}
		else {
			ui.setMenu(bwIdxAM, bandwidthAMStr, 6);
			}
	}
	
	if (digitalRead(AGC_BUT) == LOW) {
	cmdAgc = true;
	delay(50);
	}
